...
(async () => {
      ...
      $parent.innerHTML = `
            <div class="v9tJq">
                <div class="fx7hk">
                    ...
                    <a class="_9VEo1" href="clipboard.htm" data-type=""><span aria-label="태그됨" class="glyphsSpriteTag_up__outline__24__blue_5 u-__7"></span></a>
                </div>
            </div>
        `;
    };
  ...
  const grid = await (async ($parent, url) => {
        ...
        `
            <article class="FyNDV">
                <div class="Igw0E rBNOH YBx95 ybXk5 _4EzTm soMvl JI_ht bkEs3 DhRcB">
                    <button class="sqdOP L3NKy y3zKF JI_ht" type="button" id="recentSortBtn">최신순</button>
                    <button class="sqdOP L3NKy y3zKF JI_ht" type="button" id="popularSortBtn">인기순</button>
                    <h1 class="K3Sf1">
                                ...
                                <div class="Igw0E IwRSH eGOV_ ybXk5 _4EzTm">
                                    <div class="Igw0E IwRSH eGOV_ vwCYk">
                                        <label class="NcCcD">
                                            ...
                                            autocomplete="off" class="j_2Hd iwQA6 RO68f M5V28" placeholder="검색" id="searchInput" spellcheck="true" type="search" value="" />
                                            <div class="DWAFP">
    ...
    const sort = event => {
      // 현재는 각 컴포넌트가 destroy 미지원 -> 그냥 DOM만 비우고, 새로 gridItem들 생성
      $el.lastElementChild.firstElementChild.innerHTML = "";
      if (event.target.id === "recentSortBtn") {
        // 최신순
        timelineList.sort(
          (x, y) => Date.parse(x.timestamp) - Date.parse(y.timestamp)
        );
      } else if (event.target.id === "popularSortBtn") {
        //인기순
        timelineList.sort(
          (x, y) =>
            x.clipCount +
    ...
    recentSortBtn.addEventListener("click", sort);
    popularSortBtn.addEventListener("click", sort);
